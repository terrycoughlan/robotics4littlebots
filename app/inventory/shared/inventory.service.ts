/**
 * Created by coughlat on 12-Apr-17.
 */
import {EventEmitter, Injectable} from '@angular/core'
import {Subject, Observable} from 'rxjs/Rx'
import {IContract, IInventory} from "./inventory.model";
...
export class InventoryService{
    getInventories():Observable<IInventory[]>{
        let subject = new Subject<IInventory[]>()
        setTimeout(() => {subject.next(INVENTORIES); subject.complete();}, 100)
        return subject
    }
    getInventory(id:number):IInventory{
    ...
    updateInventory(inventory){
        let index = INVENTORIES.findIndex(x => x.id = inventory.id)
        INVENTORIES[index] = inventory
    }

    searchContracts(searchTerm: string){
        var term = searchTerm.toLocaleLowerCase();
        var results: IContract[] = [];

        INVENTORIES.forEach(inventory => {
            var matchingContracts = inventory.contracts.filter(contract => contract.name.toLocaleLowerCase().indexOf(term)> -1);
            matchingContracts = matchingContracts.map((contract:any) => {
                contract.inventoryId = inventory.id;
                return contract;
            })
            results = results.concat(matchingContracts);
        })
...
const INVENTORIES:IInventory[] = [
    {
        id: 1,
        name: 'Let\'s build an awesome robot',
        date: new Date ('1/10/2017'),
        time: '10:00 am',
        price: 125.00,
        imageUrl: '/app/assets/images/BGlgRob5.jpg',
        location: {
            address: 'Resorce center',
            city: 'Clonmellon',
            country: 'Ireland'
        },
        contracts: [
          ...
          overworking them. In this contract I'll show you how to get the
          best results from the talent you already have on staff.`,
            ...
            },
            {
                ...
                duration: 2,
                level: "Advanced",
                abstract: `Angular 4 Performance is hot. In this contract, we'll see
        ...
        contracts: [
          ...
          about his secretes for keeping up with the changes, and navigating around the code.`,
            ...
            },
            {
                ...
                duration: 1,
                level: "Beginner",
                abstract: `In this contract, Lukas will present the 
    ...
    },
    {
        ...
        contracts: [
          ...
          brie they make using Angular 4. This contract will show you how.`,
